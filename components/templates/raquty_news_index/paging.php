<?php 
//ページネーション
function raquty_news_paging($max){
    //該当の行数を取得
    $sql = "SELECT COUNT(post_id) AS row_count FROM post_info WHERE template = ? AND post_status = ?";
    global $cms_access;
    $stmt = $cms_access->prepare($sql);
    $template = 'raquty_news';
    $status = 'publish';
    $stmt->bind_param("si", $template, $status);
    $stmt->execute();
...
function generatePagingLinks($paging) {
    if ($paging['now'] != 1) {
        echo '<a href="' . home_url('raquty_News?page=' . ($paging['now'] - 1)) . '">&lt;&lt;</a>';
    }
        ...
        if ($paging['now'] >= 3) {
            if (($paging['num'] - $paging['now'] >= 2)) {
                for ($number = $paging['now'] - 2; $number <= $paging['now'] + 2; $number++) {
                    echo '<a href="' . ($number != $paging['now'] ? home_url('raquty_News?page=' . $number) : 'javascript:void(0)') . '" ' . ($number == $paging['now'] ? "class='active'" : '') . '>' . $number . '</a>';
                }
            } else {
